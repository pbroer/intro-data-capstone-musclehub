...
print(pval)
#a Pval of 0.00096 < 0.05 cutoff, so we reject H0.  H0 is that there is no difference between the datasets, so this result is significant.
...
chi2, pval, dof, expected = chi2_contingency(contingency)
print(pval)
#Pval of 0.43 > 0.05 so we cannot reject H0.
...
chi2, pval, dof, expected = chi2_contingency(contingency)
print(pval)
#pval of 0.014 < 0.05 so we reject H0.
#H0 is that there is no difference between the datasets, so this result is significant.
